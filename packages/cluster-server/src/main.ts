...
export async function runClusterServer(env: Readonly<Record<string, string | undefined>> = process.env): Promise<void> {
	...
	const logger = new JsonLogger(undefined, { component: "cluster-server", version: "0.1.32", namespace: config.namespace });
	const health = new ClusterServerHealth();
	const metrics = new ClusterMetrics({ component: "cluster-server", version: "0.1.32", namespace: config.namespace });
...
async function main(): Promise<void> {
	try { await runClusterServer(); }
		...
		const logger = new JsonLogger(undefined, { component: "cluster-server", version: "0.1.32" });
