...
| Windows  | x86_64                | `.msi` (**unsigned, see below**)         |

No Intel Mac build in v0.1.22. The iOS TestFlight build is coming soon.
...
> [!WARNING]
> **The Windows v0.1.22 MSI is unsigned.** Microsoft Defender SmartScreen can warn about an unrecognized app or publisher. Only continue if you trust the release from this repository. You can always build from source instead.

1. Download [`T4-Code-0.1.22-win-x64.msi`](https://github.com/LycaonLLC/t4-code/releases/download/v0.1.22/T4-Code-0.1.22-win-x64.msi).
2. Open the downloaded MSI, or install it for the current user from PowerShell:

   ```powershell
   msiexec /i ".\T4-Code-0.1.22-win-x64.msi"
   ```

3. Launch T4 Code and open a pairing link generated by the Linux or macOS OMP host you want to use.

The Windows desktop package is a remote client. Local OMP service discovery and management remain available on Linux and macOS.
...
**Local (Linux and macOS).** T4 Code looks for the `omp` executable via `$OMP_EXECUTABLE`, your `PATH`, and common install locations (`~/.local/bin`, `/usr/local/bin`, `/opt/omp/bin`, ...). It then manages one appserver per OMP profile for you: a systemd user service on Linux, a launch agent on macOS. Named profiles under `~/.omp/profiles` appear as their own local hosts and can auto-start with the app. Appserver logs land in `~/.local/state/t4-code/appserver` (Linux) or `~/Library/Logs/T4 Code/appserver` (macOS); named profiles log under `profiles/<id>` inside those directories. The Windows app connects to paired hosts instead of managing a local OMP service.
...
2. Launch T4 Code. On the same Linux or macOS machine, it finds `omp` and offers to start the appserver. On Windows or when using another machine, open the pairing link from that host.
...
pnpm package:windows  # x64 MSI into release/ (on Windows)
```
