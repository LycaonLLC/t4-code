...
test("current source tree has one consistent release version", () => {
  assert.deepEqual(collectReleaseConsistencyErrors(files, "v0.1.15"), []);
});

test("rejects a tag that differs from the package version", () => {
  assert.ok(
    collectReleaseConsistencyErrors(files, "v9.9.9").some((error) =>
      error.includes("release tag v9.9.9 does not match v0.1.15"),
    ),
  );
});
...
test("rejects workspace, site, README, and runtime version drift", () => {
  const cases = [
    ["apps/web/package.json", (text) => text.replace('"version": "0.1.15"', '"version": "0.1.3"')],
    [
      "apps/site/src/release.ts",
      (text) => text.replace('RELEASE_TAG = "v0.1.15"', 'RELEASE_TAG = "v0.1.3"'),
    ],
    ["README.md", (text) => text.replace("Download v0.1.15", "Download v0.1.3")],
    [
      "apps/desktop/src/target-manager.ts",
      (text) => text.replace('version: "0.1.15"', 'version: "0.1.3"'),
    ],
    [
...
test("rejects drift in verified OMP runtime provenance", () => {
  const cases = [
    (text) =>
      text.replace(
        "f9322817981e16bf3f1e3d77684f4269f026aa64",
        "0000000000000000000000000000000000000000",
      ),
    (text) => text.replace('"sourceTag": "t4code-16.5.2-appserver-2"', '"sourceTag": "wrong-tag"'),
    (text) =>
      text.replace(
...
test("rejects stale README release URLs while allowing historical prose", () => {
  ...
  assert.ok(
    collectReleaseConsistencyErrors(staleLink).some((error) =>
      error.includes("release URL for v0.1.3; expected v0.1.15"),
    ),
  );
