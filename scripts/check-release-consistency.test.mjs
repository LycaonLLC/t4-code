...
test("rejects a tag that differs from the package version", () => {
  assert.ok(
    collectReleaseConsistencyErrors(files, "v9.9.9").some((error) =>
      error.includes("release tag v9.9.9 does not match v0.1.26"),
    ),
  );
});
...
test("tagged releases reject published provenance drift", () => {
  ...
  for (const [field, mutate] of appWireCases) {
    ...
    assert.ok(
      collectReleaseConsistencyErrors(drifted, "v0.1.26").some((error) =>
        error.includes(
  ...
  assert.ok(
    collectReleaseConsistencyErrors(extended, "v0.1.26").some((error) =>
      error.includes(
...
test("rejects workspace, site, README, and runtime version drift", () => {
  const cases = [
    ["apps/web/package.json", (text) => text.replace('"version": "0.1.26"', '"version": "0.1.3"')],
    [
      "apps/site/src/release.ts",
      (text) => text.replace('RELEASE_TAG = "v0.1.26"', 'RELEASE_TAG = "v0.1.3"'),
    ],
    ["README.md", (text) => text.replace("Download v0.1.26", "Download v0.1.3")],
    [
      "apps/desktop/src/target-manager.ts",
      (text) => text.replace('version: "0.1.26"', 'version: "0.1.3"'),
    ],
    [
...
test("rejects stale README release URLs while allowing historical prose", () => {
  ...
  assert.ok(
    collectReleaseConsistencyErrors(staleLink).some((error) =>
      error.includes("release URL for v0.1.3; expected v0.1.26"),
    ),
  );
  assert.deepEqual(collectReleaseConsistencyErrors(files), []);
});
