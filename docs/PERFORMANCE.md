...
Session-click phases have two boundaries. Metrics named `session-dom-click` start at the actual DOM
click received by the application and are the primary product-performance signal. The older
`session-click` metrics start immediately before Playwright asks Chromium to click, so they also
include Playwright's actionability checks, pointer movement, and input delivery. The separate
`playwright-session-click-command-to-dom-click` metric makes that automation-only time visible. Do
not attribute it to application rendering or use it alone to judge a product optimization.
