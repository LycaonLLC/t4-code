...
describe("desktop read-aloud process bridge", () => {
  it("uses a private temp file and keeps text out of argv", async () => {
    const root = await mkdtemp(join(tmpdir(), "t4-speech-test-")); const child = new FakeChild(); let argv: readonly string[] = []; const spawned = Promise.withResolvers<void>();
    const service = createDesktopSpeechService({ discoverExecutable: async () => "/opt/omp", makeTempDirectory: async () => mkdtemp(join(root, "run-")), spawn: fakeSpawn(child, (_, next, options) => { argv = next; expect((options as { shell?: boolean }).shell).toBe(false); spawned.resolve(); }) });
  ...
  it("replaces and cancels active speech, settling both requests", async () => {
    ...
    const first = service.speakText({ text: "first" }); await firstSpawned.promise; const second = service.speakText({ text: "second" }); await secondSpawned.promise; expect(children[0]?.killed).toBe(true); await service.stopSpeaking(); expect(children[1]?.killed).toBe(true); expect(await first).toMatchObject({ accepted: false }); expect(await second).toMatchObject({ accepted: false }); await service.dispose(); await rm(root, { recursive: true, force: true });
  });
