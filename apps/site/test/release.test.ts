// Release contract guard: exact v0.1.23 asset names and URLs, and the
...
describe("release assets", () => {
  it("carries the five contracted v0.1.23 filenames", () => {
    expect(RELEASE_ASSETS.map((a) => a.filename)).toEqual([
      "T4-Code-0.1.23-android.apk",
      "T4-Code-0.1.23-linux-amd64.deb",
      "T4-Code-0.1.23-linux-x86_64.AppImage",
      "T4-Code-0.1.23-mac-arm64.dmg",
      "T4-Code-0.1.23-mac-arm64.zip",
    ]);
  });
  ...
  it("targets the public LycaonLLC repo", () => {
    ...
    expect(RELEASE_TAG).toBe("v0.1.23");
    expect(RELEASE_VERSION).toBe("0.1.23");
...
describe("OMP integration contract", () => {
  it("pins the verified runtime tag, commit, and app-wire package", () => {
    expect(OMP_RUNTIME_TAG).toBe("t4code-17.0.4-appserver-2");
    expect(OMP_RUNTIME_COMMIT).toBe("469bf579dca26c86ffe760a4a4e102d026279ed8");
    expect(OMP_RUNTIME_URL).toBe(
      "https://github.com/lyc-aon/oh-my-pi/tree/t4code-17.0.4-appserver-2",
    );
